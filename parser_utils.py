...
DSL_PRESETS = {
    ...
    "Conv-LSTM Hybrid": "[64, 128], conv1d: [128, 3], lstm: [128], [128, 10]",
    "Kitchen Sink": "fractal: [256, 2], moe: [256, 8], gqa: [256, 8, 2], residual: [256], conv1d: [256, 5], lstm: [256], dropout: [0.1], [256, 10]",
}
...
def parse_program(program):
    ...
    Supported layer types:
      - [in, out]              → Linear layer
      - attn: [dim]            → Self-attention
      - gqa: [dim, heads, groups] → Grouped Query Attention
      - moe: [dim, experts]    → Mixture of Experts
      - trans: [dim]           → Transformer block
      - fractal: [dim, depth]  → Fractal recursive block
      - dropout: [rate]        → Dropout regularization (v4.0)
      - residual: [dim]        → Residual FFN block (v4.0)
      - conv1d: [dim, kernel]  → 1D convolution block (v4.0)
      - lstm: [dim, layers]    → BiLSTM block (v4.0)
    """
    # Strip comments and extra whitespace
    clean_prog = "\n".join([line.split("#")[0] for line in program.splitlines()])
    
    number = Word(nums).setParseAction(lambda t: int(t[0]))
    # Support float numbers for dropout rate
    ...
    moe_layer = Group(Suppress("moe") + colon + open_bracket + number + Optional(comma + number) + close_bracket).set_parse_action(
        lambda t: {
            'type': 'moe', 'dim': t[0][0], 'experts': t[0][1] if len(t[0]) > 1 else 8
        }
    )
    ...
    fractal_layer = Group(Suppress("fractal") + colon + open_bracket + number + Optional(comma + number) + close_bracket).set_parse_action(
        lambda t: {
            'type': 'fractal', 'dim': t[0][0], 'depth': t[0][1] if len(t[0]) > 1 else 2
        }
    )
    
    # --- v4.0 NEW layer definitions ---
    ...
    residual_layer = Group(Suppress("residual") + colon + open_bracket + number + Optional(comma + number) + close_bracket).set_parse_action(
        lambda t: {
            'type': 'residual', 'dim': t[0][0], 'expansion': t[0][1] if len(t[0]) > 1 else 4
        }
    )
    conv1d_layer = Group(Suppress("conv1d") + colon + open_bracket + number + Optional(comma + number) + close_bracket).set_parse_action(
        lambda t: {
            'type': 'conv1d', 'dim': t[0][0], 'kernel': t[0][1] if len(t[0]) > 1 else 3
        }
    )
    lstm_layer = Group(Suppress("lstm") + colon + open_bracket + number + Optional(comma + number) + close_bracket).set_parse_action(
        lambda t: {
            'type': 'lstm', 'dim': t[0][0], 'layers': t[0][1] if len(t[0]) > 1 else 1
        }
    )
    
    # Order matters: more specific (keyword) layers before the generic linear layer
    layer = (fractal_layer | moe_layer | gqa_layer | attn_layer | trans_layer | 
             dropout_layer | residual_layer | conv1d_layer | lstm_layer | linear_layer)
    
    # Force full string consumption with StringEnd
...
def validate_dsl(program):
    ...
    if result is None:
        issues.append(("ERROR", "Failed to parse DSL. Check syntax: brackets, commas, layer keywords."))
        return issues, None
    ...
    for i, layer_def in enumerate(result):
        ...
        if lt == 'linear':
            if prev_out is not None and layer_def['in'] != prev_out:
                issues.append(("WARNING", f"Layer {i}: input dim {layer_def['in']} doesn't match previous output {prev_out}"))
            prev_out = layer_def['out']
        elif lt == 'dropout':
            pass  # Dropout doesn't change dims
        elif lt in ('fractal', 'moe', 'gqa', 'attn', 'trans', 'residual', 'conv1d', 'lstm'):
            dim = layer_def.get('dim')
            if prev_out is not None and dim != prev_out:
                issues.append(("WARNING", f"Layer {i} ({lt}): dim {dim} doesn't match previous output {prev_out}"))
            prev_out = dim
